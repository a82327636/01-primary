...
public class JedisConfig {
    ...
    @Value("${jedis.args.host}")
    private String host;
    @Value("${jedis.args.port}")
    private int port;
    @Value("${jedis.args.password}")
    private String password;

    /**
     * 缓存生存时间秒
     */
    private final int expire = 60 * 10 * 6;     //1小时
    /**
     * 操作Key的方法
     */
    public Keys KEYS;
    ...
    /**
     * 加一个@bean是为了执行init方法，因为spring ioc会初始化加载
     */
    @Bean
    public void init(){
        ...
        jedisPool = new JedisPool(config, host, port, 10000, StringUtils.isNotBlank(password) ? password : null);
    }

    public Jedis getJedis() {
        // 只有一个jedispool,ioc容器管理的只有一个
     ...
     * @param time
     */
    public void addObjectNx(String key, Object value, int time) {
        Jedis jedis = getJedis();
        jedis.setnx(key.getBytes(), SerializeUtil.serialize(value));
        jedis.expire(key, time);
        returnJedis(jedis);
    }
